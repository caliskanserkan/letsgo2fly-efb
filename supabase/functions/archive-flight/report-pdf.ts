...
export interface ReportInput {
  ...
  ftlEnabled?: boolean;
  // Kapali hucrelerin satirlari hic cizilmez. 12 Agu 2026'dan itibaren bu
  // UCUSUN DONMUS konfigurasyonudur (`flt_report.features_snapshot`), sirketin
  // BUGUNKU ayari degil — yoksa REGEN gecmis rapordan satir dusururdu (Ilke 3).
  features?: Record<string, boolean>;
  // Konfigurasyonun kaynagi: 'device' | 'server' | null. Alt bilgiye basilir —
  // denetci, elindeki raporun hangi konfigurasyonla uretildigini gormeli.
  featuresSource?: string | null;
}
...
export async function buildReportPdf(input: ReportInput): Promise<Uint8Array> {
  ...
  // ── Alt bilgi ─────────────────────────────────────────────────────────────
  // MODUL KONFIGURASYONUNUN KAYNAGI (12 Agu 2026): rapor sunucuda ve ISTENILDIGI
  // ZAMAN yeniden uretilebildigi icin, denetcinin elindeki kopyanin HANGI
  // konfigurasyonla basildigi kagidin uzerinde yazmali. "Not recorded" hali
  // gizlenmez: o rapor hicbir bolumu budamamis demektir (Ilke 1).
  const CFG_NOTE: Record<string, string> = {
    device: "Module config: flight snapshot",
    server: "Module config: live at archive time",
  };
  const cfgNote = CFG_NOTE[String(input.featuresSource ?? "")]
    ?? "Module config: not recorded - no section suppressed";
  ensure(c, 20);
  c.y -= 8;
  txt(c, `Report generated by GO2 eFB . Archive copy . CAMO data not included . ${cfgNote} . ${new Date().toUTCString()}`,
