    ...
    <div style={{display:'flex',flex:1,overflow:'hidden'}}>
          ...
          {detailTab==='details'&&(
            <>
              ...
              <div style={{borderTop:`1px solid ${C.border}`}}>
                ...
                <CollapsibleEditBox title="Flight Crew" icon="**" color="#1a9bc4" logs={logsByCategory.crew} fields={[{key:'pf_id',label:'PF Pilot',type:'pilot'},{key:'sic_id',label:'PM Pilot',type:'pilot'}]} flight={sel} onSave={load} toast={toast} user={user} pilots={pilots}/>
                <CollapsibleEditBox title="Mandatory" icon="OK" color="#2d9e5f" logs={logsByCategory.mandatory} fields={[]} flight={sel} onSave={load} toast={toast} user={user} pilots={pilots}/>
                <CollapsibleEditBox title="Fuel" icon="F" color="#2d9e5f" logs={logsByCategory.fuel} fields={[{key:'takeoff_fuel',label:'T/O Fuel (lb)',type:'text'},{key:'remaining_fuel',label:'Rem Fuel (lb)',type:'text'}]} flight={sel} onSave={load} toast={toast} user={user} pilots={pilots}/>
                ...
                <CollapsibleEditBox title="NAV LOG" icon="NAV" color="#1a9bc4" logs={logsByCategory.navlog} fields={[{key:'off_blocks',label:'Off Blocks (HH:MM)',type:'time'},{key:'takeoff_time',label:'T/O Time (HH:MM)',type:'time'},{key:'landing_time',label:'Landing (HH:MM)',type:'time'},{key:'on_blocks',label:'On Blocks (HH:MM)',type:'time'},{key:'takeoff_fuel',label:'T/O Fuel (lb)',type:'text'},{key:'remaining_fuel',label:'Rem Fuel (lb)',type:'text'}]} flight={sel} onSave={load} toast={toast} user={user} pilots={pilots}/>
                <CollapsibleEditBox title="LND Data" icon="LND" color="#1a9bc4" logs={logsByCategory.lnd} fields={[{key:'arr_rwy',label:'ARR RWY',type:'text'},{key:'arr_atis',label:'ARR ATIS',type:'text'},{key:'actual_lw',label:'Actual LW (lb)',type:'text'},{key:'vref',label:'Vref (kt)',type:'text'},{key:'req_landing_dist',label:'Req LND Dist',type:'text'}]} flight={sel} onSave={load} toast={toast} user={user} pilots={pilots}/>
              </div>
      ...
      {editModal&&sel&&(
        <Modal title="EDIT ARCHIVED FLIGHT — REPORT REQUIRED" onClose={()=>setEditModal(false)} width={520}>
          <div style={{fontSize:11,color:'#e8731a',marginBottom:16,padding:'8px 12px',background:'rgba(232,115,26,0.08)',border:'1px solid rgba(232,115,26,0.2)'}}>All edits are logged. Only changed fields will be updated.</div>
      ...
      {deleteModal&&sel&&(
        <Modal title="DELETE ARCHIVED FLIGHT RECORD" onClose={()=>setDeleteModal(false)}>
          <div style={{fontSize:12,color:'#e02020',marginBottom:16,padding:'10px 12px',background:'rgba(224,32,32,0.08)',border:'1px solid rgba(224,32,32,0.2)'}}>Irreversible. Record will be permanently deleted and logged.</div>
...
function Aircrafts({toast}){
  ...
  return(
    <div style={{display:'flex',flex:1,overflow:'hidden'}}>
      ...
      {sel&&!editing&&(()=>{ const t=getTotals(sel); const appHours=t.appMins>0?`${Math.floor(t.appMins/60)}:${String(t.appMins%60).padStart(2,'0')}`:'0:00'; return(<DetailPanel title="Aircraft Detail" onClose={()=>setSelected(null)}><DetailRow label="Registration" value={sel.registration} accent/><DetailRow label="Manufacturer" value={sel.manufacturer}/><DetailRow label="Model" value={sel.model}/><DetailRow label="ICAO Type" value={sel.ac_type}/><DetailRow label="Landing Cat" value={sel.landing_cat}/><div style={{padding:'9px 16px',borderBottom:`1px solid ${C.border}`,background:C.bg3}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}><span style={{fontSize:12,color:C.t3,fontFamily:"'Courier New',monospace"}}>TOTAL HOURS</span><span style={{fontSize:15,color:C.accent,fontFamily:"'Courier New',monospace",fontWeight:700}}>{t.hours}</span></div><div style={{fontSize:10,color:'#555',fontFamily:"'Courier New',monospace"}}>Baseline: {sel.total_hours||0}h + App logged: {appHours}</div></div><div style={{padding:'9px 16px',borderBottom:`1px solid ${C.border}`,background:C.bg3}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}><span style={{fontSize:12,color:C.t3,fontFamily:"'Courier New',monospace"}}>TOTAL CYCLES</span><span style={{fontSize:15,color:C.accent,fontFamily:"'Courier New',monospace",fontWeight:700}}>{t.cycles}</span></div><div style={{fontSize:10,color:'#555',fontFamily:"'Courier New',monospace"}}>Baseline: {sel.total_cycles||0} + App logged: {t.appCycles}</div></div><DetailRow label="Status" value={sel.active?'Active':'Inactive'}/><div style={{padding:'12px 16px'}}><button style={{...S.btnPrimary,width:'100%'}} onClick={openEdit}>EDIT AIRCRAFT</button></div></DetailPanel>); })()}
...
function Crews({toast}){
  ...
  const handleAddCrew=async()=>{ if(!addForm.full_name||!addForm.code||!addForm.email||!addForm.password){toast('All fields required.','error');return;} setSaving(true); try{ const url=`${process.env.REACT_APP_SUPABASE_URL}/auth/v1/admin/users`; const svcKey=process.env.REACT_APP_SUPABASE_SERVICE_KEY; if(!svcKey){toast('REACT_APP_SUPABASE_SERVICE_KEY not set in .env','error');setSaving(false);return;} const res=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json','apikey':svcKey,'Authorization':`Bearer ${svcKey}`},body:JSON.stringify({email:addForm.email,password:addForm.password,email_confirm:true,user_metadata:{full_name:addForm.full_name,code:addForm.code}})}); const authUser=await res.json(); if(!res.ok){toast(authUser.msg||'Auth user creation failed','error');setSaving(false);return;} const{error:profErr}=await supabase.from('profiles').upsert({id:authUser.id,full_name:addForm.full_name,code:addForm.code.toUpperCase(),email:addForm.email,role:addForm.role},{onConflict:'id'}); if(profErr){toast(`Profile error: ${profErr.message}`,'error');setSaving(false);return;} toast(`${addForm.full_name} added successfully.`,'success'); setShowAdd(false); setAddForm({full_name:'',code:'',email:'',role:'pilot',password:''}); load(); }catch(e){toast(e.message,'error');} setSaving(false); };
  const handleDeleteStep1=(pilot)=>{setDeleteTarget(pilot);setDeleteModal(true);setDeleteConfirm(false);};
  const handleDeleteConfirm=async()=>{ if(!deleteTarget)return; setSaving(true); try{ const url=`${process.env.REACT_APP_SUPABASE_URL}/auth/v1/admin/users/${deleteTarget.id}`; const svcKey=process.env.REACT_APP_SUPABASE_SERVICE_KEY; await fetch(url,{method:'DELETE',headers:{'apikey':svcKey,'Authorization':`Bearer ${svcKey}`}}); await supabase.from('crew_qualifications').delete().eq('pilot_id',deleteTarget.id); await supabase.from('profiles').delete().eq('id',deleteTarget.id); toast(`${deleteTarget.full_name} deleted.`,'success'); setDeleteModal(false);setDeleteTarget(null);setSelected(null);load(); }catch(e){toast(e.message,'error');} setSaving(false); };
  ...
  const efbStatus=rec=>{ if(!rec?.efb_training_date)return{color:'#e02020',label:'NO RECORD'}; if(!rec.efb_training_valid_until)return{color:'#2d9e5f',label:'CURRENT'}; const d=Math.floor((new Date(rec.efb_training_valid_until)-new Date())/86400000); if(d<0)return{color:'#e02020',label:'EXPIRED'}; if(d<30)return{color:'#e8731a',label:`${d}d LEFT`}; return{color:'#2d9e5f',label:'CURRENT'}; };
  return(
    <div style={{display:'flex',flex:1,overflow:'hidden'}}>
      <div style={{flex:1,overflowY:'auto'}}>
...
function SortableTable({flights, fmt}){
  ...
  const icon=(key)=>{ if(sortKey!==key)return<span style={{color:'#333',marginLeft:4}}>⇅</span>; return<span style={{color:C.accent,marginLeft:4}}>{sortDir==='asc'?'↑':'↓'}</span>; };
  return(<table style={S.table}><thead><tr>{COLS.map(c=>(<th key={c.key} style={{...S.th,cursor:'pointer',userSelect:'none'}} onClick={()=>toggle(c.key)}>{c.label}{icon(c.key)}</th>))}</tr></thead><tbody>{sorted.map(f=>(<tr key={f.id}><td style={S.td}>{f.archived_at?new Date(f.archived_at).toLocaleDateString('en-GB'):'—'}</td><td style={{...S.td,color:C.accent}}>{f.departure_icao||f.plans?.dep||'—'}</td><td style={{...S.td,color:C.accent}}>{f.destination_icao||f.plans?.dest||'—'}</td><td style={S.td}>{f.plans?.reg||'—'}</td><td style={S.td}>{fmt(f.block_minutes)}</td><td style={S.td}>{fmt(f.airborne_minutes)}</td><td style={S.td}>{f.landing_count||'—'}</td><td style={S.td}>{f.is_night_landing?<span style={S.badge('blue')}>NIGHT</span>:'—'}</td></tr>))}</tbody></table>);
}
...
function StationInfo({toast}){
  ...
  const riskBadge=(level)=>{ const textColors={LOW:'#4a9bc4',MEDIUM:'#e8a320',HIGH:'#e8731a',EXTREME:'#e02020'}; return(<span style={{fontSize:11,fontWeight:700,padding:'2px 8px',background:level==='LOW'?C.blueDim:level==='MEDIUM'?'#2a1a00':level==='HIGH'?'#1a0a00':'#1a0000',color:textColors[level]||'#888',border:`1px solid ${textColors[level]||'#444'}`}}>{level||'—'}</span>); };
...
function RiskAssessmentInline({icao, onClose}){
  ...
  const RISK_C={LOW:{bg:'rgba(26,155,196,0.12)',border:'#1a9bc4',text:'#1a9bc4'},MEDIUM:{bg:'rgba(232,163,32,0.12)',border:'#e8a320',text:'#e8a320'},HIGH:{bg:'rgba(232,115,26,0.12)',border:'#e8731a',text:'#e8731a'},EXTREME:{bg:'rgba(224,32,32,0.12)',border:'#e02020',text:'#e02020'}};
  ...
  const tabS=(t)=>({flex:1,padding:'8px 4px',textAlign:'center',cursor:'pointer',fontFamily:"'Courier New',monospace",fontSize:10,fontWeight:700,letterSpacing:1,textTransform:'uppercase',color:tab===t?'#1a9bc4':'#555',borderBottom:tab===t?'2px solid #1a9bc4':'2px solid transparent',background:'transparent',border:'none'});
  const ScoreInput=({val,onChange})=>(<select value={val} onChange={e=>onChange(parseInt(e.target.value))} style={{background:'#1a1a1a',border:'1px solid #383838',color:'#e8e8e8',fontSize:11,padding:'3px 6px',borderRadius:3,fontFamily:"'Courier New',monospace",width:44}}>{[1,2,3,4,5].map(n=><option key={n} value={n}>{n}</option>)}</select>);
  return(
    <div style={{fontFamily:"'Courier New',monospace",color:'#e8e8e8'}}>
      <div style={{padding:'14px 18px',background:'#1a1a1a',borderBottom:'1px solid #2a2a2a',display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
        <div><div style={{fontSize:20,fontWeight:700,color:'#e8a020',letterSpacing:2}}>{ap.icao}</div><div style={{fontSize:13,color:'#e8e8e8',marginTop:2}}>{ap.name}</div><div style={{fontSize:10,color:'#555',marginTop:3}}>CAT {editing?catEdit:ap.category||'B'}{ap.ad_elev_ft?` · ${ap.ad_elev_ft} ft`:''}</div></div>
      ...
      {editing&&(<div style={{padding:'8px 12px',background:'#161616',borderBottom:'1px solid #2a2a2a',display:'flex',gap:16,alignItems:'center'}}><div style={{fontSize:10,color:'#555',fontWeight:700,textTransform:'uppercase',letterSpacing:1}}>Category</div><select value={catEdit} onChange={e=>setCatEdit(e.target.value)} style={{background:'#1a1a1a',border:'1px solid #383838',color:'#e8e8e8',fontSize:11,padding:'4px 8px',borderRadius:3}}><option value="A">A</option><option value="B">B</option><option value="C">C</option></select><div style={{fontSize:10,color:'#555',fontWeight:700,textTransform:'uppercase',letterSpacing:1}}>Mitigation</div><input value={mitEdit} onChange={e=>setMitEdit(e.target.value)} style={{...S.input,flex:1,fontSize:11}} placeholder="Brief mitigation note..."/></div>)}
      <div style={{display:'flex',gap:2,margin:'12px 12px 0',background:'#2a2a2a',padding:2}}>
        ...
        <div style={{flex:2,background:'#1a1a1a',border:`2px solid ${rc.border}`,padding:'12px 14px'}}><div style={{fontSize:18,fontWeight:800,color:rc.text}}>{rl}</div><div style={{fontSize:10,color:rc.text,opacity:.8,marginTop:4}}>{getOps(rl,editing?catEdit:ap.category)}</div></div>
        <div style={{flex:2,background:'#1a1a1a',border:'2px solid #2a2a2a',padding:'12px 14px',fontSize:10,color:'#777',lineHeight:1.8}}><div>MAX S: <span style={{color:'#e8e8e8',fontWeight:700}}>{maxSVal}</span></div><div>MAX L: <span style={{color:'#e8e8e8',fontWeight:700}}>{maxLVal}</span></div><div style={{marginTop:4,color:'#555',fontSize:9}}>{(editing?mitEdit:ap.mitigation||'').slice(0,60)}</div></div>
      </div>
      <div style={{margin:'10px 12px 0',background:'#1a1a1a',border:'1px solid #2a2a2a',padding:'10px 12px'}}><div style={{fontSize:9,color:'#555',fontWeight:700,letterSpacing:1,marginBottom:8,textTransform:'uppercase'}}>Operasyonel Faktörler</div><div style={{display:'flex',flexWrap:'wrap',gap:6}}>{ADDONS_LIST.map(a=>(<div key={a.key} onClick={()=>setAd(p=>({...p,[a.key]:!p[a.key]}))} style={{cursor:'pointer',padding:'5px 10px',borderRadius:4,fontSize:10,fontWeight:700,background:addons[a.key]?'rgba(232,115,26,0.2)':'#252525',border:`1px solid ${addons[a.key]?'#e8731a':'#383838'}`,color:addons[a.key]?'#e8731a':'#555'}}>{addons[a.key]?'✓':'+'} {a.label} (+{a.pts})</div>))}</div></div>
      <div style={{display:'flex',margin:'10px 12px 0',borderBottom:'1px solid #2a2a2a'}}><button style={tabS('matrix')} onClick={()=>setTab('matrix')}>5×5 Matrix</button><button style={tabS('topics')} onClick={()=>setTab('topics')}>{editing?'✏ Edit Scores':'Topic Scores'}</button><button style={tabS('briefing')} onClick={()=>setTab('briefing')}>PPS Briefing</button></div>
      <div style={{margin:'0 12px 12px',background:'#1a1a1a',border:'1px solid #2a2a2a',overflowX:'auto'}}>
        {tab==='matrix'&&(<div style={{padding:12}}><div style={{fontSize:9,color:'#555',marginBottom:8}}>Mevcut: S={maxSVal} × L={maxLVal} → Base {baseScore} · Total {total}</div><div style={{display:'grid',gridTemplateColumns:'24px repeat(5,1fr)',gap:2,marginBottom:2}}><div style={{fontSize:9,color:'#444',textAlign:'center'}}>L\S</div>{[1,2,3,4,5].map(sv=><div key={sv} style={{fontSize:9,color:'#555',textAlign:'center',fontWeight:700}}>S{sv}</div>)}</div>{[5,4,3,2,1].map(lv=>(<div key={lv} style={{display:'grid',gridTemplateColumns:'24px repeat(5,1fr)',gap:2,marginBottom:2}}><div style={{fontSize:9,color:'#555',textAlign:'center',fontWeight:700,alignSelf:'center'}}>L{lv}</div>{[1,2,3,4,5].map(sv=>{const cs=lv*sv;const cc=cellC(cs);const isCur=lv===maxLVal&&sv===maxSVal;return<div key={sv} style={{background:cc.bg,border:isCur?`2px solid ${rc.border}`:'1px solid #2a2a2a',borderRadius:3,padding:'6px 0',textAlign:'center',fontSize:isCur?13:11,fontWeight:isCur?800:600,color:cc.text}}>{isCur?'▶':''}{cs}</div>;})})</div>))}</div>)}
        {tab==='topics'&&(<div style={{padding:8}}>{editing&&(<div style={{padding:'6px 8px',marginBottom:4,background:'rgba(232,115,26,0.08)',border:'1px solid rgba(232,115,26,0.2)',fontSize:10,color:'#e8731a'}}>S = Severity (1-5) · L = Likelihood (1-5) · Max score auto-calculated</div>)}{TOPICS_LIST.map((topic,i)=>{ const sv=editing?sEdit[i]||1:parseFloat(sArr[i])||0; const lv=editing?lEdit[i]||1:parseFloat(lArr[i])||0; const score=Math.round(sv*lv); return(<div key={i} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 8px',borderBottom:'1px solid #222'}}><div style={{flex:1,fontSize:10,color:'#999'}}>{i+1}. {topic}</div>{editing?(<><ScoreInput val={sv} onChange={v=>setSEdit(p=>{const n=[...p];n[i]=v;return n;})}/><div style={{fontSize:9,color:'#444'}}>×</div><ScoreInput val={lv} onChange={v=>setLEdit(p=>{const n=[...p];n[i]=v;return n;})}/></>):(<><div style={{background:sColor(sv),color:'#fff',borderRadius:3,padding:'2px 6px',fontSize:10,fontWeight:700,width:28,textAlign:'center'}}>S{sv}</div><div style={{fontSize:9,color:'#444'}}>×</div><div style={{background:sColor(lv),color:'#fff',borderRadius:3,padding:'2px 6px',fontSize:10,fontWeight:700,width:28,textAlign:'center'}}>L{lv}</div></>)}<div style={{fontSize:11,fontWeight:700,color:sColor(Math.max(sv,lv)),width:28,textAlign:'right'}}>{score}</div></div>); })}</div>)}
        {tab==='briefing'&&(<div style={{padding:12}}>{[{title:'SECTION 1 — Traffic / ATC / Taxi / RWY Ops',key:'section1'},{title:'SECTION 2 — Meteorology / Wind',key:'section2'},{title:'SECTION 3 — Security / Handling / Nav',key:'section3'}].map(sec=>ap[sec.key]?(<div key={sec.key} style={{marginBottom:12}}><div style={{fontSize:9,color:'#1a9bc4',fontWeight:700,letterSpacing:1,marginBottom:6,textTransform:'uppercase'}}>{sec.title}</div><div style={{fontSize:11,color:'#aaa',lineHeight:1.8,whiteSpace:'pre-line',padding:'8px 10px',background:'#151515',borderLeft:'2px solid rgba(26,155,196,0.2)'}}>{ap[sec.key]}</div></div>):null)}<div style={{marginTop:16,borderTop:'1px solid #2a2a2a',paddingTop:12}}><div style={{fontSize:9,color:'#1a9bc4',fontWeight:700,letterSpacing:1,marginBottom:8,textTransform:'uppercase'}}>Assessment History</div><AssessmentHistory icao={icao}/></div></div>)}
      </div>
    </div>
  );
}
