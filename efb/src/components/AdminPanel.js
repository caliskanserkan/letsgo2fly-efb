...
function DetailRow({ label, value, accent }) {
  return (
    <div style={{ padding: '8px 16px', borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <span style={{ fontSize: 9, color: C.t3, fontWeight: 700, letterSpacing: 1.5, fontFamily: "'Courier New', monospace", textTransform: 'uppercase' }}>{label}</span>
      <span style={{ fontSize: 11, color: accent ? C.accent : C.t1, fontFamily: "'Courier New', monospace", fontWeight: accent ? 700 : 400 }}>{value || '—'}</span>
    </div>
  );
}
