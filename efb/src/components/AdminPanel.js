  ...
  return(
    <div style={{fontFamily:"'Courier New',monospace",color:'#e8e8e8'}}>
      ...
      <div style={{margin:'0 12px 12px',background:'#1a1a1a',border:'1px solid #2a2a2a',overflowX:'auto'}}>
        ...
        {tab==='briefing'&&(<div style={{padding:12}}>{[{title:'SECTION 1 — Traffic / ATC / Taxi / RWY Ops',key:'section1'},{title:'SECTION 2 — Meteorology / Wind',key:'section2'},{title:'SECTION 3 — Security / Handling / Nav',key:'section3'}].map(sec=>ap[sec.key]?(<div key={sec.key} style={{marginBottom:12}}><div style={{fontSize:9,color:'#1a9bc4',fontWeight:700,letterSpacing:1,marginBottom:6,textTransform:'uppercase'}}>{sec.title}</div><div style={{fontSize:11,color:'#aaa',lineHeight:1.8,whiteSpace:'pre-line',padding:'8px 10px',background:'#151515',borderLeft:'2px solid rgba(26,155,196,0.2)'}}>{ap[sec.key]}</div></div>):null)}</div>)}
      <div style={{marginTop:16,borderTop:'1px solid #2a2a2a',paddingTop:12}}>
  <div style={{fontSize:9,color:'#1a9bc4',fontWeight:700,letterSpacing:1,marginBottom:8,textTransform:'uppercase'}}>Assessment History</div>
  <AssessmentHistory icao={icao}/>
  </div>
  );
}
