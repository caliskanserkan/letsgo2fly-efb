...
function TakeoffData({ setStatus, activePlan }) {
  ...
  useEffect(() => {
    ...
    logEvent(activePlan.id, 'TKOF_RWY_SELECTED', { rwy: selRwy });
  }, [selRwy]); // eslint-disable-line

  // Log: DEP ATIS (debounced 2s)
  useEffect(() => {
    if (!depAtis || !activePlan?.id) return;
    clearTimeout(atisTimer.current);
    atisTimer.current = setTimeout(() => {
      logEvent(activePlan.id, 'TKOF_ATIS_ENTERED', { dep_atis: depAtis });
    }, 2000);
    return () => clearTimeout(atisTimer.current);
  }, [depAtis]); // eslint-disable-line

  // Log: speeds (once, when V1+VR+V2 all set)
  useEffect(() => {
    if (!v1 || !vr || !v2) { speedsLogged.current = false; return; }
    if (!activePlan?.id || speedsLogged.current) return;
    speedsLogged.current = true;
    logEvent(activePlan.id, 'TKOF_SPEEDS_ENTERED', { v1, vr, v2, vse, trim });
  }, [v1, vr, v2, vse, trim]); // eslint-disable-line

  // Log: ATC CLR (debounced, when SID set)
  useEffect(() => {
    if (!sid || !activePlan?.id) return;
    clearTimeout(sidTimer.current);
    sidTimer.current = setTimeout(() => {
      logEvent(activePlan.id, 'TKOF_ATC_CLR', { sid, fl, sq, oth });
    }, 2000);
    return () => clearTimeout(sidTimer.current);
  }, [sid, fl, sq, oth]); // eslint-disable-line

  // Log: RVSM ground check (once, when all 3 set)
  useEffect(() => {
    if (!rvsm1 || !rvsmSby || !rvsm2) { rvsmLogged.current = false; return; }
    if (!activePlan?.id || rvsmLogged.current) return;
    rvsmLogged.current = true;
    logEvent(activePlan.id, 'TKOF_RVSM_GROUND', { rvsm1, rvsm_sby: rvsmSby, rvsm2 });
  }, [rvsm1, rvsmSby, rvsm2]); // eslint-disable-line
  // ── end logEvent ─────────────────────────────────────────────────────────
  ...
  return (
    <div>
      ...
      {noData && (
        <div style={{ background:'#2a2a2a', borderBottom:'1px solid #383838', padding:'10px 16px' }}>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            <input value={manualRwy} onChange={e => setManualRwy(e.target.value.toUpperCase())} placeholder="RWY" style={{ ...iStyle, width:90, textAlign:'center' }} />
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'11px 16px', background:'#2a2a2a', borderBottom:'1px solid #383838' }}>
        <span style={{ fontSize:12.5, color:'#666' }}>Stop Margin {selectedRwy ? `(RWY ${selectedRwy.id})` : manualRwy ? `(RWY ${manualRwy})` : ''}</span>
