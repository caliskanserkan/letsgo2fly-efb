      ...
      <div style={S.wrap} id="flt-report-content">
        ...
        <div className="no-print" style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16,flexWrap:'wrap',gap:8}}>
          ...
          <div style={{display:'flex',gap:8}}>
            <button onClick={()=>{ const html='<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{font-family:monospace;padding:16px;background:#fff;color:#000;font-size:11px;} table{width:100%;border-collapse:collapse;margin-bottom:12px;} th,td{padding:5px 8px;border:1px solid #ccc;font-size:10px;text-align:left;} th{background:#f1f5f9;font-weight:700;} h2{font-size:13px;margin:12px 0 4px;border-bottom:2px solid #1e3a5f;padding-bottom:4px;} .grid{display:grid;grid-template-columns:1fr 1fr 1fr;border:1px solid #e2e8f0;margin-bottom:12px;} .cell{padding:8px 10px;border:1px solid #e2e8f0;} .lbl{font-size:8px;color:#94a3b8;margin-bottom:2px;} .val{font-size:11px;font-weight:700;} @media print{body{padding:0;}}</style></head><body>'+document.getElementById('flt-report-content').innerHTML+'</body></html>'; const blob=new Blob([html],{type:'text/html;charset=utf-8'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.target='_blank'; a.rel='noopener'; document.body.appendChild(a); a.click(); document.body.removeChild(a); setTimeout(()=>URL.revokeObjectURL(url),10000); }} style={{background:'rgba(255,255,255,0.1)',border:'1px solid #475569',borderRadius:6,padding:'8px 16px',fontSize:12,color:'#fff',cursor:'pointer'}}>🖨 Print</button>
