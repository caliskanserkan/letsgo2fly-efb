...
// Custom div icon for waypoints
function wptIcon(label, color, bg, size=28) {
  return L.divIcon({
    className: '',
    html: `<div style="
      background:${bg};
...
function aptColor(type) {
  if (type === 'DEPARTURE')   return { color:'#fbbf24', bg:'rgba(251,191,36,0.15)' };
  if (type === 'DESTINATION') return { color:'#4ade80', bg:'rgba(74,222,128,0.15)' };
  if (type === 'ALTERNATE')   return { color:'#fb923c', bg:'rgba(251,146,60,0.15)' };
  return { color:'#a78bfa', bg:'rgba(167,139,250,0.15)' };
}

export default function EnrouteMap({ waypoints = [], wxAirports = [], gpsPos, liveWxMap = {} }) {

  const routeCoords = waypoints.filter(w => w.coord).map(w => [w.coord.lat, w.coord.lon]);
  ...
  return (
    <div style={{ flex:1, position:'relative', overflow:'hidden', minHeight:500 }}>
      <MapContainer center={center} zoom={zoom}
        ...
        {/* Enroute waypoints — blue tile with white label */}
        {waypoints.filter(w => w.coord && w.type === 'wpt').map(w => (
          <Marker
            key={w.uid}
            position={[w.coord.lat, w.coord.lon]}
            icon={wptIcon(w.name, '#ffffff', 'rgba(56,189,248,0.85)', w.name.length * 7)}
          >
            <Tooltip direction="top" offset={[0,-8]}>
              <span style={{fontFamily:'monospace',fontSize:11}}>
                {w.name} {w.eta !== '—' ? `· ETA ${w.eta}` : ''}
              </span>
            </Tooltip>
          </Marker>
        ))}
        ...
        {/* DEP marker */}
        {waypoints.filter(w => w.coord && w.type === 'dep').map(w => (
          <Marker
            key={w.uid}
            position={[w.coord.lat, w.coord.lon]}
            icon={wptIcon(`✈ ${w.name}`, '#0f172a', '#fbbf24', 60)}
          >
            <Tooltip permanent direction="top" offset={[0,-10]}>
        ...
        {/* DEST marker */}
        {waypoints.filter(w => w.coord && w.type === 'dest').map(w => (
          <Marker
            key={w.uid}
            position={[w.coord.lat, w.coord.lon]}
            icon={wptIcon(`⬛ ${w.name}`, '#0f172a', '#4ade80', 60)}
          >
            <Tooltip permanent direction="top" offset={[0,-10]}>
        ...
        {wxAirports.map(apt => {
          const wpt = waypoints.find(w => w.name === apt.icao || w.uid === apt.icao);
          if (!wpt?.coord) return null;
          const { color, bg } = aptColor(apt.type);
          return (
            <Marker
              key={apt.icao}
              position={[wpt.coord.lat, wpt.coord.lon]}
              icon={wptIcon(apt.icao, color, bg, apt.icao.length * 8)}
            >
              <Tooltip direction="top" offset={[0,-8]}>
                <div style={{fontFamily:'monospace',fontSize:11}}>
                  <span style={{fontWeight:700,color}}>{apt.icao}</span>
                  <span style={{color:'#666',marginLeft:4}}>{apt.type}</span>
                  {liveWxMap[apt.icao] && <div style={{fontSize:9,color:'#4ade80'}}>● LIVE WX</div>}
                </div>
              </Tooltip>
            </Marker>
          );
        ...
        {gpsPos && (
          <>
            <CircleMarker
              center={[gpsPos.lat, gpsPos.lon]}
              radius={12}
              pathOptions={{ color:'#4ade80', fillColor:'#4ade80', fillOpacity:0.3, weight:2 }}
            />
            <Marker
              position={[gpsPos.lat, gpsPos.lon]}
              icon={wptIcon('✈ ACFT', '#0f172a', '#4ade80', 56)}
            >
              <Tooltip permanent direction="top" offset={[0,-10]}>
                <span style={{fontFamily:'monospace',fontSize:10,color:'#4ade80'}}>
                  {gpsPos.lat.toFixed(3)}N {gpsPos.lon.toFixed(3)}E ±{Math.round(gpsPos.acc||0)}m
                </span>
              </Tooltip>
            </Marker>
            <FlyTo pos={gpsPos} />
