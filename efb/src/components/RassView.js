...
function AirportCard({ role, icao, data, checked, onCheck }) {
  ...
  return (
    <div style={{ flex:1, minWidth:0, background:C.bg2, border:`1px solid ${checked?'#4ade80':rm.border}`, borderRadius:12, overflow:'hidden', transition:'border-color 0.2s' }}>
      ...
      {data ? <>
        {[['CATEGORY',data.category?`CAT ${data.category}`:'—'],['ELEVATION',data.ad_elev_ft?`${data.ad_elev_ft} FT`:'—'],['MAX S',data.max_s??'—'],['MAX L',data.max_l??'—']].map(([l,v])=>(
          <div key={l} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 14px', borderBottom:`1px solid ${C.border}` }}>
            ...
            <span style={{ fontSize:11, color:'#f1f5f9', fontFamily:"'Courier New',monospace", fontWeight:700 }}>{v}</span>
          </div>
        ))}
        {data.ops_approval && (
      ...
      <div onClick={() => onCheck && onCheck(!checked)}
        style={{ display:'flex', alignItems:'center', gap:10, padding:'13px 14px', background:checked?'rgba(74,222,128,0.08)':'rgba(255,255,255,0.02)', borderTop:`1px solid ${checked?'#4ade80':C.border}`, cursor:'pointer', transition:'background 0.15s', minHeight:48 }}>
        <div style={{ width:22, height:22, borderRadius:6, flexShrink:0, border:`2px solid ${checked?'#4ade80':'#334155'}`, background:checked?'#4ade80':'transparent', display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.15s' }}>
          {checked && <span style={{ color:'#0f172a', fontSize:13, lineHeight:1, fontWeight:700 }}>✓</span>}
        </div>
        <span style={{ fontSize:11, color:checked?'#4ade80':C.t3, fontFamily:"'Courier New',monospace", fontWeight:checked?700:400 }}>
