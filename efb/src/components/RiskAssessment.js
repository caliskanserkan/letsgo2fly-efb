...
const cellC = (s) => s>=20?{bg:'#3a0808',text:'#f06060'}:s>=12?{bg:'#2a1200',text:'#e8731a'}:s>=6?{bg:'#0a1a00',text:'#6db890'}:{bg:'#0a1a2a',text:'#4a9bc4'};
...
export function RiskAssessment({ icao, onClose }) {
  ...
  if (!icao) return null;
  if (loading) return <div style={{padding:24,textAlign:'center',color:'#555',fontFamily:"'Courier New',monospace"}}>LOADING {icao}...</div>;
  ...
  return (
    <div style={{fontFamily:"'Courier New',monospace",color:'#e8e8e8',background:'#111'}}>
      <div style={{padding:'14px 18px',background:'#1a1a1a',borderBottom:'1px solid #2a2a2a',display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
        ...
        {onClose && <button onClick={onClose} style={{background:'none',border:'none',color:'#555',cursor:'pointer',fontSize:20}}>x</button>}
      </div>
      ...
      <div style={{margin:'10px 12px 0',background:'#1a1a1a',border:'1px solid #2a2a2a',padding:'10px 12px'}}>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
          {ADDONS.map(a=>(
            <div key={a.key} onClick={()=>setAd(p=>({...p,[a.key]:!p[a.key]}))} style={{cursor:'pointer',padding:'5px 10px',borderRadius:4,fontSize:10,fontWeight:700,background:addons[a.key]?'rgba(232,115,26,0.2)':'#252525',border:`1px solid ${addons[a.key]?'#e8731a':'#383838'}`,color:addons[a.key]?'#e8731a':'#555'}}>
      ...
      <div style={{margin:'0 12px 12px',background:'#1a1a1a',border:'1px solid #2a2a2a'}}>
        {tab==='matrix'&&(
          <div style={{padding:12}}>
            ...
            {[5,4,3,2,1].map(lv=>(
              <div key={lv} style={{display:'grid',gridTemplateColumns:'24px repeat(5,1fr)',gap:2,marginBottom:2}}>
                ...
                {[1,2,3,4,5].map(sv=>{const cs=lv*sv;const cc=cellC(cs);const isCur=lv===maxL&&sv===maxS;return <div key={sv} style={{background:cc.bg,border:isCur?`2px solid ${rc.border}`:'1px solid #2a2a2a',borderRadius:3,padding:'6px 0',textAlign:'center',fontSize:isCur?13:11,fontWeight:isCur?800:600,color:cc.text}}>{isCur?'> ':''}{cs}</div>;})}
              </div>
            ))}
          </div>
        )}
          ...
          <div style={{padding:8}}>
            {TOPICS.map((topic,i)=>{const sv=parseFloat(s[i])||0;const lv=parseFloat(l[i])||0;const score=Math.round(sv*lv);return(
              <div key={i} style={{display:'flex',alignItems:'center',gap:8,padding:'6px 8px',borderBottom:'1px solid #222'}}>
                ...
                <div style={{fontSize:9,color:'#444'}}>x</div>
