...
// Multi-strategy: ___/___ marker (primary) + FIR4 pattern (fallback)
function parseWaypoints(rawText, dep, dest, std) {
  ...
  for (let i = 0; i < lines.length - 1; i++) {
    ...
    // Strategy A: line contains ___/___ (PPS OFP ETA/ATA blank field)
    if (line1.includes('___/___')) {
      const m = line1.match(/^\s+\S+\s+([A-Z][A-Z0-9]{1,5})\s/);
      if (m) {
        rawName = m[1];
        // TFREM: last 3-5 digit number on line
        const tf = line1.match(/\b(\d{3,5})\s*$/);
        planFuel = tf ? parseInt(tf[1]) : null;
      }
    }
    // Strategy B: AWY WPT FIR4 pattern (no ___/___)
    else {
      const m = line1.match(/^\s+\S+\s+([A-Z][A-Z0-9]{1,5})\s+[A-Z]{4}\s/);
      if (m) {
        rawName = m[1];
        const tf = line1.match(/\b(\d{3,5})\s*$/);
        planFuel = tf ? parseInt(tf[1]) : null;
    ...
    if (!rawName) continue;
    if (rawName === 'TOC' || rawName === 'TOD' || rawName.includes('-')) continue;

    // ATM: search next 1-3 lines for H:MM pattern
    let atmMins = null;
    ...
    // Only store if not already found (keep first occurrence)
    if (!routeData[rawName]) {
      routeData[rawName] = { eta: fromMins(stdMins + atmMins), planFuel };
    }
  }

  // Fuzzy name match: "C2000" ↔ "C2000F", "C3558" ↔ "C3558A" etc.
              ...
              {doDiv&&(
                <div style={{marginTop:10,display:'flex',flexDirection:'column',gap:10}}>
                  ...
                  <div><div style={{fontSize:10,color:'#e8731a',fontWeight:700,letterSpacing:0.6,textTransform:'uppercase',marginBottom:6}}>Runway</div>
                    <input value={divRwy} onChange={e=>setDivRwy(e.target.value.toUpperCase())} placeholder="e.g. 27L"
...
function NavLog({ flightData, updateFlight, setStatus, activePlan, updateDivert }) {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        ...
        console.warn('NavLog OFP parse:',e);
        if(!waypoints.length)setWaypoints([
          {uid:dep,name:dep,type:'dep',eta:std||'—',fl:'—',planFuel:null,custom:false,coord:null},
          {uid:dest,name:dest,type:'dest',eta:'—',fl:'—',planFuel:null,custom:false,coord:null},
        ]);
  ...
  const onAddWptAt=(nw,pos2,rel)=>{setWaypoints(p=>{const i=p.findIndex(w=>w.uid===rel);if(i===-1)return p;const u=[...p];u.splice(pos2==='before'?i:i+1,0,nw);return u;});setModal(null);};
  ...
  const fuelDev=(wpt,actNum)=>{if(!actNum)return null;const pl=planFuelAt(wpt);if(!pl)return null;return actNum-pl;};
            ...
            <React.Fragment key={wpt.uid}>
              <div onClick={()=>!sk&&setModal(wpt.uid)}
                ...
                <div style={{fontSize:11,fontFamily:'monospace'}}>
                  {actATA?(
                    <div><span style={{color:'#2d9e5f',fontWeight:700}}>{actATA}</span>{df&&<span style={{fontSize:9,color:df.color,marginLeft:3,fontWeight:700}}>{df.label}</span>}</div>
                  ):estATA?(
        ...
        <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.85)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200}}>
          <div style={{background:'#1a1a1a',border:'2px solid #e8731a',borderRadius:12,width:340,overflow:'hidden'}}>
            ...
            <div style={{padding:'0 16px 16px',display:'flex',gap:8}}>
              ...
              <button onClick={()=>{setGpsOk(true);setShowGpsWarn(false);startGPS();logEvent(activePlan?.id,'GPS_ACTIVATED',{notice:'NOT_FOR_NAVIGATION_acknowledged'});}}
      ...
      {modal&&modal!==dep&&mWpt&&mWpt.type==='wpt'&&(()=>{
        ...
        return(
          <WptModal wpt={mWpt} onClose={()=>setModal(null)}
            ...
            onAddWpt={(nw,p2)=>onAddWptAt(nw,p2,modal)}
