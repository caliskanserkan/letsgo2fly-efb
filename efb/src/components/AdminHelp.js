  ...
  },
  {
    ...
    sections: [
      ...
      { h: 'DUTY HISTORY & EDIT REPORT', qa: [
        ...
        ['A past duty still says PLN — the flight was flown but the EFB never archived it.',
         'Click the PLN badge. It asks whether the planned duty took place. YES opens the actual times — OFF BLOCK, T/O, LND and ON BLOCK per sector, UTC; off-block and on-block are mandatory because the duty window, FDP and the earliest next report are computed from them, with the duty\'s OWN ruleset snapshot. It saves as ACTUAL/finished. The sector carries entered_manually and NO plan_id, so if the real archive turns up later its measured times replace the typed ones.'],
        ['And if the planned duty never happened?',
         'Answer NO. The duty is CANCELLED — not deleted: it stays in the record and the audit log, and drops out of the roster and the totals. Then say what the day actually was: EMPTY DAY, STANDBY or OFF. That last step matters — the rules count days, so a day left as nothing is a day the ruleset cannot reason about.'],
        ['Why does STANDBY ask for hours?',
         'Because a standby without hours does not exist, and without a start and an end the rules cannot shorten the next max FDP or set the rest that follows (SHT-FTL/HG Md.17). You also pick which standby it was — AIRPORT (Md.17/1) or OTHER (Md.17/2) — since the two reduce differently. The day is written as a GND duty exactly as if it had been rostered.'],
        ['Who can do this, and is it logged?',
         'Only in the editable panel — a pilot reading their own report sees a plain PLN badge and cannot change anything. Every path demands a reason and writes to the edit log BEFORE the change, same as EDIT/CANCEL/DELETE.'],
        ['What is EDIT REPORT?',
    ...
    sections: [
      { h: 'AERODROME DATABASE', qa: [
        ...
        ['What do categories A, B and C mean?',
         'A: standard aerodrome — self-briefing suffices. B: additional briefing items apply (RAAQ items 1-4). C: aerodrome posing specific problems for approach, landing or take-off — item 5 also applies and special training completed by the aircrew is required. All five items are always PRINTED on the card; the category decides which of them are ticked. Without a category nothing can be ticked at all — see below.'],
      ]},
      { h: 'THE FOP-FRM-02 FORM ON THE PILOT CARD', qa: [
        ['What is on every RAAQ card now?',
         'The full paper form, in the paper order. (1) FOLLOWING ITEMS WERE BRIEFED AND FAMILIARIZED FOR THE ROUTE FLOWN — seven items, no boxes, identical at every aerodrome. (2) SPECIAL ITEMS BRIEFED DUE TO AERODROME CATEGORY — all five items printed, with boxes. (3) SPECIAL REMARKS — the PPS briefing sections, at the BOTTOM. Then the review confirmation. This applies whether or not a risk survey exists: until 19 Aug 2026 the special items were shown ONLY when there was no survey, so a surveyed aerodrome produced no qualification form at all.'],
        ['What does a completed risk survey add?',
         'The RISK ASSESSMENT MATRIX here produces a score, a risk level (LOW/MEDIUM/HIGH/EXTREME), an ops-approval requirement and three narrative PPS briefing sections (traffic/ATC, meteorology, security/handling). Those sections appear inside SPECIAL REMARKS, read-only, with the assessor and date. With no survey the box says NO PPS BRIEFING ON FILE rather than being left empty — an empty box cannot be told apart from a display fault.'],
        ['Which boxes get ticked, and when?',
         'On "reviewed": items 1-4 for CAT A/B, items 1-5 for CAT C. Item 5 is printed for every aerodrome but stays empty unless the aerodrome is CAT C — exactly like the paper form. What was ticked is recorded with the category, the pilot and the timestamp (rass_data.raaq + the RASS_REVIEWED audit event).'],
        ['What happens when the aerodrome has no category?',
         'The card carries a banner at the TOP with the ICAO code: AERODROME CATEGORY NOT SET — CONTACT DISPATCH. No box can be ticked, the review confirmation is LOCKED, no RAAQ record is written, and the RAAQ module CANNOT TURN GREEN — which also means Accept & Sign cannot be completed. The category decides which items apply, so without it the form simply cannot be filled in. Set the category here and the pilot can proceed.'],
      ]},
      { h: 'TROUBLESHOOTING', qa: [
        ['Pilot reports "CATEGORY NOT SET" for an aerodrome.',
         'This one blocks departure: with no category the RAAQ module cannot turn green, so Accept & Sign cannot be completed. Open RAAQ here, find the aerodrome and set its category (or ADD AIRPORT if it is missing). The pilot card unlocks on the next open — no app restart needed.'],
